            ...
            for(auto &entry : window.entries) {
                ...
                if(entry.type == toggle){
                    ...
                    if(ImGui::Checkbox(entry.text.c_str(), &toggleMap[entry.text])){
                        client->SendUpdate(entry.text, toggle, toggleMap[entry.text], 0.f);
                    }
                }
                else if(entry.type == button){
                    if(ImGui::Button(entry.text.c_str())) {
                        client->SendUpdate(entry.text, button, false, 0.f);
                    }
                }
                else if(entry.type == slider){
                    static float value = entry.slider.start;
                    if(ImGui::SliderFloat(entry.text.c_str(), &value, entry.slider.min, entry.slider.max)){
                        client->SendUpdate(entry.text, slider, false, value);
                    }
                }
            }
            ImGui::End();
